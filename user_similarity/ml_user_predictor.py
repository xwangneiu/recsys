...
def predict(dataset, dest_filename):
    ...
    for i in range(len(users_and_items)):
        ...
        item_ratings = None
        if item not in um_a2p_ix:
            prediction = math.nan
        else:
            item_ratings = um[um_a2p_ix[item], :]
            
            #process into dictionary ratings
            #ratings keys are ACTUAL 
            ratings = {}
            for j in range(len(item_ratings)):
                if not np.isnan(item_ratings[j]):
                    ratings[sm_p2a_ix[j]] = item_ratings[j]
            #print(ratings)
            #print(item_ratings)
            sim_user = None
            if user not in sm_a2p_ix:
                ...
                sim_user = sm[sm_a2p_ix[user], :]
                
                similarities = {}
                for j in range(len(sim_user)):
                    if not np.isnan(sim_user[j]):
                        if sm_p2a_ix[j] in ratings:
                            similarities[sm_p2a_ix[j]] = sim_user[j]
                #print(similarities)
